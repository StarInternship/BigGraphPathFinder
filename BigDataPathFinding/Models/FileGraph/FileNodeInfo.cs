...
namespace BigDataPathFinding.Models.FileGraph
{
    public class FileNodeInfo : NodeInfo
    {
        ...
        {
            InputAdjacent.Add(new Adjacent(inputId, weight));
        }

        public void AddOutput(Guid outputId, double weight)
        {
            OutputAdjacent.Add(new Adjacent(outputId, weight));
        }

        public IEnumerable<Adjacent> GetAllAdjacent()
        {
            return InputAdjacent.Union(OutputAdjacent);
        }
    }
}